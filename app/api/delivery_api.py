...
delivery_service = SellerService()


@deliver_router.post("/signup", response_model=BaseSellerSchema, status_code=status.HTTP_201_CREATED)
async def create_seller_account(req: CreateSellerSchema, session: SessionDep):
    return await delivery_service.register_seller(req_body=req, session=session)


@deliver_router.get("/", response_model=List[BaseSellerSchema], status_code=status.HTTP_200_OK)
async def get_all_sellers(session: SessionDep):
    return await delivery_service.get_all_sellers(session=session)
